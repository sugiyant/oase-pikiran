...
async function run() {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    const prompt = `Write an extremely extensive and deeply analytical article in Bahasa Indonesia for Oase Pikiran.
Title: ${item.title}
...
- Must be between 750 and 950 words. Ensure there are no short summaries!`;
  ...
  const reviewPrompt = `Review the following article based on Oase Pikiran's Editorial Constitution.
Criteria:
