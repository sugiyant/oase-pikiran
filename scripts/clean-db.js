...
async function clean() {
  ...
  const res = execSync('npx wrangler d1 execute oase-pikiran-db --remote --command "SELECT slug, content FROM articles"', { encoding: 'utf8' });
  try {
    ...
    for (const row of rows) {
      ...
      if (modified) {
        ...
        execSync(`npx wrangler d1 execute oase-pikiran-db --remote --file="${tempFile}"`);
