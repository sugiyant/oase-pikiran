...
async function fixForeignArticles() {
  ...
  const rawD1 = execSync('set -a; [ -f ~/.env ] && . ~/.env; npx wrangler d1 execute oase-pikiran-db --remote --command="SELECT slug, title, content, category FROM articles;"', { encoding: "utf8" });
...
Bagaimana kita dapat menghargai hubungan kita dengan alam di tengah kecepatan modernisasi yang sering kali mengabaikan akar ekologis kita?`
  ...
  execSync(`set -a; [ -f ~/.env ] && . ~/.env; npx wrangler d1 execute oase-pikiran-db --remote --file="${batchFile}"`, { stdio: "inherit" });
