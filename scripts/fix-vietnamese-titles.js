...
async function run() {
  ...
  let res;
  try {
    res = execSync(`npx wrangler d1 execute oase-pikiran-db --remote --command "${query}"`, { encoding: 'utf8' });
  } catch (err) {
  ...
  for (const art of articles) {
    ...
    let contentRes;
    try {
      contentRes = execSync(`npx wrangler d1 execute oase-pikiran-db --remote --command "SELECT content, category, published_at FROM articles WHERE slug = '${escapeSQL(art.slug)}'"`, { encoding: 'utf8' });
    } catch(e) {
    ...
    try {
      ...
      execSync(`npx wrangler d1 execute oase-pikiran-db --remote --file="${delFile}"`, { stdio: 'ignore' });
      // Then insert
      execSync(`npx wrangler d1 execute oase-pikiran-db --remote --file="${insFile}"`, { stdio: 'ignore' });
