...
async function run() {
  ...
  const existingRes = execSync('npx wrangler d1 execute oase-pikiran-db --remote --command "SELECT slug, title FROM articles"', { encoding: 'utf8' });
  ...
  while (count < 50) {
    ...
    if (batchQueries.length > 0) {
      ...
      try {
        execSync(`npx wrangler d1 execute oase-pikiran-db --remote --file="${tempFile}"`, { stdio: 'inherit' });
      } catch (err) {
