...
export async function generateText(prompt, { json = false, temperature = 0.6 } = {}) {
  const messages = [
    {
      role: "system",
      content: "You are Oase Pikiran's editorial AI. Follow EDITORIAL_CONSTITUTION.md. Truth-first, constructive-second. Never invent facts, quotations, sources or statistics. When uncertain, say so or omit the claim. IMPORTANT: Output raw Markdown directly. NEVER wrap the article text in ```markdown ... ``` code blocks."
    },
    {
      role: "user",
      content: prompt
